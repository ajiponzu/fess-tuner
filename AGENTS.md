...
- 検索・ヘルスチェック API: `FESS_BASE_URL/api/v1/`（認証不要・公開 API）
- 管理 API: `FESS_BASE_URL/api/admin/`（`Authorization: <token>` ヘッダー、Bearer 不要）
- 接続情報は `.env` で管理（`FESS_BASE_URL`, `FESS_ACCESS_TOKEN`）
