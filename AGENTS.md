...
ローカルまたはネットワーク上に構築した Fess サーバー（Docker 等）に対して、
...
- サーバー URL: `.env` の `FESS_BASE_URL` で管理する
  - `.env`のテンプレは下記
  ```
  FESS_BASE_URL=http://<fess-host>:8080
...
- `FESS_BASE_URL` には接続先 Fess の URL を記載する（例: `http://localhost:8080`, `http://<server-name>:8080`）
- スクリプト側の未設定時 fallback はローカル実行向けの `http://localhost:8080` とする
